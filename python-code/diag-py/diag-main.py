...
def do_qa_func(func, fBase):
    ...
    debug = 1
    import getQABoardsInfo.py as qa
    if g_src == "":
        g_src = qa.strTestFile
    cInfo = qa.CQaBrdsInfo(g_src, debug)
    dTorList = cInfo.ParserFile(qa.strTorFile)
    dEorList = cInfo.ParserFile(qa.strEorFile)
...
def do_prod_func(func,fBase):
    ...
    if g_funcType == (funcBase + 1):   # get board information
        dSearch = cInfo.SearchItem(g_keysLst, 0)
        cInfo.printSearch(dSearch)
    elif g_funcType == (funcBase + 2):   # get board some information
        dSearch = cInfo.SearchItem(g_keysLst, 1)
        cInfo.printSearch(dSearch)
    elif g_funcType == (funcBase + 3):   # save data to csv file
        print("save information")
        cInfo.writeAllBrds2Csv(g_dst)
    elif g_funcType == (funcBase + 4):   # print all boards
        cInfo.printAllBoards()
    elif g_funcType == (funcBase + 5):   # print header
        cInfo.printHeader()
...
def do_pexpect_func(func, fBase):
    global g_brdLst, g_src, g_dst
    import pexpect_telnet.py as pt
...
if __name__ == '__main__':
    ...
    elif g_funcType > 7 and g_funcType <= 10:   # get
