...
class CProductInfo(object):
    ...
    def findBoardsFromWeb(self):
        ...
        #print(soup)
        divs = soup.findAll("div", {"class": "divTableWidth"})
        #divs = soup.findAll(attrs={"class":re.compile(r"^divRow ")})
        divs = soup.findAll(attrs={"style":re.compile(r"^dis")})
        divs = soup.findAll(attrs={"title":re.compile(r"^La")})
        #soup1 = BeautifulSoup(str(divs))
        #divs = soup1.findAll("div", {"class": "name"})
        print("========", divs)
        idx = 0
        for div in divs:
            idx = idx + 1
...
if __name__ == "__main__":
    ...
    url = r'http://172.31.236.9/html/webtools/prodinfotbl.htm'
    url = r"http://172.31.236.9/html/eor_web.htm"
    parserOpt()
